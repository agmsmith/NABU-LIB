...
## NthPongCustomisations Modified Fork

This NthPongCustomisations branch by Alexander G. M. Smith has some corrections to reduce warnings (add void to function declarations with no arguments), white space improvements, and rebases updates from the main fork.  It's functionally the same as the main NABU-LIB up to the commit "Remove extra #warning empty lines."  After that I start changing things, making it more optimal for Nth Pong Wars, including removing the text buffer in RAM to save space (CP/M has a text buffer too, so that's redundant).  See the git log to find what I "improved" in the quest for more RAM for my ever-growing game (see the [Nth Pong Wars Blog](https://web.ncf.ca/au829/WeekendReports/20240207/NthPongWarsBlog.html) for that story).
...
# LICENSE
This is a library for a 40 year old computer - so there's no license other than asking for fair acknowledgement/credit of my effort that got you this far. The only thing I ask is if you could give me credit if you use it. If you make changes to NABULIB or my programs, please send them my way so I can update this repo with your additions. You can find me on the NABU forum at https://forums.nabu.ca. Remember, this is a hobby for all of us, not a business so share and enjoy!
